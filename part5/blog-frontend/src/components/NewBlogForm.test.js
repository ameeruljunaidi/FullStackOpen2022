...
describe("Testing new blog form", () => {
    ...
    beforeEach(async () => {
        ...
        await act(async () => {
            ...
            root.render(
                <NewBlogForm handleNewBlog={handleNewBlogMockHandler} />
            );
        });
    });
    ...
    test("form calls the event handler received as props with the right details when a new blog ic created", async () => {
        ...
        expect(handleNewBlogMockHandler.mock.calls[0]).toContain(
            "this is a test title"
        );
        expect(handleNewBlogMockHandler.mock.calls[0]).toContain(
            "this is a test author input"
        );
        expect(handleNewBlogMockHandler.mock.calls[0]).toContain(
            "this is a test url input"
        );
    });
});
